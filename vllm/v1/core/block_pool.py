...
class BlockPool:
    ...
    def get_cached_block(
        ...
        """
        cached_blocks = []
        # 遍历组中存在相同内容的block，存在一个没有缓存，则直接返回
    ...
    def free_blocks(self, ordered_blocks: Iterable[KVCacheBlock]) -> None:
        ...
        Args:
            ordered_blocks: A list of blocks to free ordered by their eviction
                priority.
        """ 
        # 当一个block的引用异常之后，会怎样？ 无法加入到队列中
        # 释放时，对应的block也要释放自己的引用
        # 将批量blocks加入
        freeBlocks = []
        firstBlock = KVCacheBlock(-1)
        for block in ordered_blocks:
            ...
            if block.ref_cnt == 0 and not block.is_null:
                firstBlock.next_free_block = block
                block.prev_free_block = firstBlock
                firstBlock = block
                freeBlocks.append(block)
        if freeBlocks:
            self.free_block_queue.appends(freeBlocks)
