...
class BlockPool:
    ...
    def _free_delay_blocks(self):
        ...
        for col in range(max_len - 1, -1, -1):
            for row in range(len(self.delay_free_blocks) - 1, -1, -1):
                blocks = self.delay_free_blocks[row]
                blocks_len = len(blocks)
                if col < blocks_len and blocks[blocks_len - col].ref_cnt == 0:
                    self.free_block_queue.append(blocks[blocks_len - col])
